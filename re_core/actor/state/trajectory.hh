...
#include "re_core/actor/state/rigid_body_state.hh"
...
// RigidBodyState<FSE3> objects describing the state of the body at that time.
...
//
// Under the hood the trajectory is represented by a TCurve<FSE3>. The TCurve
...
class Trajectory {
 public:
  ...
  // will be thrown on invalid timestamps.
  RigidBodyState<FSE3> point_at(const time::Timestamp &at_time) const;

  // Query the state of the rigid body at a given time duration from the start.
  // Note the timestamp must be within the valid range of the trajectory. Errors
  // will be thrown on invalid timestamps.
  RigidBodyState<FSE3> point_at(const time::Duration &from_start) const;

  // Getter for the underlying TCurve.
  const curves::TCurve<FSE3> &curve() const { return curve_; }
